...
Status Manager::PrefetchScanWithEstimatesExperimental(
    const Key& start_key, const size_t amount,
    std::vector<std::pair<Key, std::string>>* values_out) {
  // Scan strategy (all scans are forward scans):
  // - Find segment containing starting key.
  // - Estimate how much of the segment to read based on the position of the
  // key.
  // - Estimate how many pages we need to read in total.
  // - Prefetch all pages in segments.
  // - Scan forward, waiting on pages to be read.
  //
  // Locking strategy:
  // - No locks acquired (this implementation is experimental and not designed
  // to run concurrently with writes).
  values_out->clear();
  values_out->reserve(amount);
  ...
  if (first_segment_page_count > 1) {
    ...
    if (page_idx >= first_segment_page_count) {
      ...
      start_page_idx = page_idx;
      est_start_pages_to_read = first_segment_page_count - start_page_idx;
    }
  }

  std::vector<std::future<std::pair<char*, size_t>>> ready_pages;
  PageBuffer& prefetch_buf = w_.prefetch_buffer();
  size_t buf_pages_left = Workspace::kPrefetchBufferPages;
  char* page_buf = prefetch_buf.get();

  assert(buf_pages_left >= est_start_pages_to_read);
  ready_pages.emplace_back(
      bg_threads_->Submit([this, start_seg, start_page_idx,
                           est_start_pages_to_read, buf = page_buf]() {
        const std::unique_ptr<SegmentFile>& sf =
            segment_files_[start_seg.sinfo.id().GetFileId()];
        const size_t segment_byte_offset =
            start_seg.sinfo.id().GetOffset() * Page::kSize;
        sf->ReadPages(segment_byte_offset + start_page_idx * Page::kSize, buf,
                      est_start_pages_to_read);
        w_.BumpReadCount(est_start_pages_to_read);
        return std::make_pair(buf, est_start_pages_to_read);
      }));
  page_buf += Page::kSize * est_start_pages_to_read;
  buf_pages_left -= est_start_pages_to_read;
  ...
  while (records_left > 0 && curr_seg.has_value()) {
    ...
    const size_t est_pages_left = std::ceil(
        records_left / static_cast<double>(options_.records_per_page_goal));
    assert(buf_pages_left >= seg_page_count);

    ready_pages.emplace_back(bg_threads_->Submit(
        [this, curr_seg, seg_byte_offset, seg_page_count, buf = page_buf]() {
          const std::unique_ptr<SegmentFile>& sf =
              segment_files_[curr_seg->sinfo.id().GetFileId()];
    ...
    page_buf += Page::kSize * seg_page_count;
    buf_pages_left -= seg_page_count;
