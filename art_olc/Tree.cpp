...
Tree::Tree(LoadKeyFunction loadKey)
    : root(new N256(nullptr, 0)), loadKey(loadKey) {}
...
bool Tree::lookupRange(const Key& start, const Key& end, Key& continueKey,
  ...
  std::function<void(N*, uint8_t, uint32_t, const N*, uint64_t)> findStart =
      [&copy, &start, &findStart, &toContinue, this](
        ...
        readAgain:
          ...
          prefixResult =
              checkPrefixCompare(node, start, 0, level, loadKey, needRestart);
          if (needRestart) goto readAgain;
  ...
  std::function<void(N*, uint8_t, uint32_t, const N*, uint64_t)> findEnd =
      [&copy, &end, &toContinue, &findEnd, this](
          N* node, uint8_t nodeK, uint32_t level, const N* parentNode,
          uint64_t vp) {
        if (N::isLeaf(node)) {
          return;
        ...
        readAgain:
          ...
          prefixResult =
              checkPrefixCompare(node, end, 255, level, loadKey, needRestart);
          if (needRestart) goto readAgain;
...
bool Tree::lookupRange(const Key& start, TID result[], std::size_t resultSize,
                       ...
                       Key* continueKey, uint64_t* index_locked_already) const {
  EpocheGuard epocheGuard(threadEpocheInfo);
  TID toContinue = 0;
  std::function<void(const N*)> copy = [&result, &resultSize, &resultsFound,
    ...
    if (N::isLeaf(node)) {
      ...
      if (cache_entries != nullptr) {
        ...
        if (index_locked_already == nullptr || *index_locked_already != index)
...
inline typename Tree::CheckPrefixResult Tree::checkPrefix(N* n, const Key& k,
                                                          uint32_t& level) {
  if (n->hasPrefix()) {
