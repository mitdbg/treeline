...
struct Options {
  ...
  // The maximum size of TL's buffer pool, in bytes.
  ...
  // The number of background threads TL should use (must be at least 2).
  // TL uses one background thread to coordinate flushing the memtable and
  // needs at least one other background thread to run the flush work.
  unsigned background_threads = 4;

  // If true, TL will pin the background threads to cores `0` to
  // `background_threads - 1`.
  bool pin_threads = true;

  // If true, TL will try to optimize the FlushOptions for every flush.
  ...
  // If true, TL will try to optimize the memory allocation between the buffer
  ...
  // If false, TL will use a BTreeModel instead of an ALEXModel.
  bool use_alex = true;
  ...
  // If true, TL will print messages to a debug log.
...
struct WriteOptions {
  // If true, TL will optimize inserts for bulk loading sorted keys.
  ...
  // If true, the write will not be written to TL's write-ahead log.
  //
