...
U64Dataset U64Dataset::GenerateOrdered(size_t size_mib, size_t record_size,
                                       uint64_t start_key, uint64_t step_size) {
  if (step_size == 0) {
  ...
  if (total_dataset_size % record_size != 0) {
    throw std::invalid_argument(
        "The requested dataset size is not divisible by the record size.");
  }
  const size_t num_records = total_dataset_size / record_size;
  const size_t value_size = record_size - sizeof(uint64_t);
