...
// node's parent.
 ...
 public:
  ...
  // Directly returns a pointer to the payload found through upper_bound(key)
  P* get_payload_upper_bound(const T& key) const {
    stats_.num_lookups++;
    data_node_type* leaf = get_leaf(key);
    int idx = leaf->find_upper(key);

    if (idx == leaf->data_capacity_) {
      if (leaf->next_leaf_) {
        // Edge case: need to check next data node
        data_node_type* next_leaf = leaf->next_leaf_;
        return &(next_leaf->get_payload(next_leaf->first_pos()));
      } else {
        return nullptr;
      }
    } else {
