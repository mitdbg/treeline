...
TEST(RecordCacheTest, SimplePutGet) {
  ...
  uint64_t index_out;
  ASSERT_TRUE(rc.GetIndex(key, false, &index_out).ok());
...
TEST(RecordCacheTest, SimpleMiss) {
  ...
  uint64_t index_out;
  ASSERT_TRUE(rc.GetIndex(key, false, &index_out).IsNotFound());
}
...
TEST(RecordCacheTest, PutVariants) {
  ...
  uint64_t index_out;
  ASSERT_TRUE(rc.GetIndex(key1, false, &index_out).ok());
  ASSERT_EQ(value.compare(rc.cache_entries[index_out].GetValue()), 0);
  ...
  ASSERT_TRUE(rc.GetIndex(key2, false, &index_out).ok());
  ASSERT_EQ(value.compare(rc.cache_entries[index_out].GetValue()), 0);
  ...
  ASSERT_TRUE(rc.GetIndex(key3, false, &index_out).ok());
...
TEST(RecordCacheTest, MultiPutGet) {
  ...
  for (auto i = 190; i < 200; ++i) {
    ...
    ASSERT_TRUE(rc.GetIndex(Slice(key_s), false, &index_out).ok());
