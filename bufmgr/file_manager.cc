...
FileManager::FileManager(const size_t page_size, const bool use_direct_io) {
  database_fd_ =
      open("database.dat", O_CREAT | O_RDWR | (use_direct_io ? O_DIRECT : 0),
           S_IREAD | S_IWRITE);

  if (database_fd_ < 0) perror("Database file could not be opened");
...
void FileManager::ReadPage(const uint64_t page_id, void* data) {
  if (page_id >= next_page_id_) ZeroOut();
  size_t offset = page_id * page_size_;
  ssize_t ret = pread(database_fd_, data, page_size_, offset);
...
void FileManager::ZeroOut() {
  ...
  int ret = ftruncate(database_fd_, new_total_length);
